...
def get_references(md_content: str, source_name: str = "Tài liệu") -> tuple[list[str], str]:
    ...
    ref_data = re.sub(r'\|', '', ref_data)
    ref_data = re.sub(r'\*\*==> picture.*?<==\*\*', '', ref_data)
    ref_data = re.sub(r'PLOS ONE.*?(?=\n|$)', '', ref_data)
...
def detect_format(ref_data: str) -> str:
    if re.search(r'^\s*(?:\*\*)?\d+\.(?:\*\*)?\s+', ref_data, re.MULTILINE):
        return 'plos'
    if re.search(r'^\s*(?:-\s*|\*\*\s*)*\\?\[\d+\\?\]', ref_data, re.MULTILINE):
        return 'ieee'
...
def split_refs(ref_data: str, fmt: str) -> list[str]:
    ...
    if fmt == 'plos':
        ...
        parts = re.split(r'\s+-\s+(?=[A-Z][a-z])', ref_data)
    else:
...
def clean_parts(parts: list[str]) -> list[str]:
    ...
    for p in parts:
        ...
        p = re.sub(r'(?i)(https?://[^\s]+)\s+([a-z0-9])', r'\1\2', p) 
